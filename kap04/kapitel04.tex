...
Chapter~3, the surrogate violation estimate must dominate the true
...
Chapter~3. Second, for each candidate decision $x$, form the residuals
...
Chapter~4: one smooth inequality, size independent of $N$, gradients by
the chain rule.
...
the one promised in Chapter~4's closing: the unbiased reformulation is
...
along three routes whose trade-offs this section fixes, because the
...
establish. Removing that catch is the purpose of the next section.
...
Chapter~3, and a solver started cold against it inherits a rugged,
