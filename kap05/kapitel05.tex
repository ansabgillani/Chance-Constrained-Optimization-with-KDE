...
Chapter~4 closed on a measured verdict: the unbiased reformulation is an
accuracy method, not a safety method, and its certificate fails in $39\%$ of
the standalone diagnostic trials. This chapter converts the verdict into a
construction.
...
property previewed in Chapter~3's kernel figure and now developed in
...
\subsubsection{Directional failure of a symmetric surrogate}
\label{sec:directionalfailure}
...
estimator of Chapter~4 is consistent, its bandwidth theory is classical,
...
certificate being granted. Chapter~4 measured the result at
$39\%$ certificate failure.

\subsubsection{Fixed-decision envelope versus optimizer selection}
\label{sec:fixeddecisionenvelope}
...
bounded by the indicator it replaces. For a fixed decision $x$ independent
of the sample, this gives the expectation bound
$\E[\hat{p}_h^{\,b}(x)] \leq \Prob(g(x,\xi) \leq 0)$.
bound. It does not by itself transfer to the data-dependent optimizer returned
by the nonlinear program; a selection-aware concentration argument or an
independent validation set is still required. Equivalently, in the violation
form of Chapter~3, the per-sample surrogate dominates the empirical
violation indicator; this does not make the population violation probability
pointwise observable. Sampling and optimizer selection can still produce an
unsafe population decision.

\subsection{The Biasing Construction}
\label{sec:biasing}

\subsubsection{Pointwise domination condition}
\label{sec:pointwisedomination}
...
of Chapter~4, a valid CDF bounded in $[0,1]$ that violates the
...
\cref{eq:domination} asks for exactly one thing beyond Chapter~4: the
...
\subsubsection{Expectation bound and its qualification}
\label{sec:expectationbound}

The conservative estimator and constraint follow immediately. Averaging
the dominated surrogate over the residual samples of Chapter~4 gives
\begin{equation}
  \hat{p}_h^{\,b}(x)
  \;=\; \frac{1}{N}\sum_{j=1}^{N}
  ...
  \label{eq:biasedcc}
\end{equation}
For a fixed decision $x$ independent of the sample, each term of
\cref{eq:biasedcc} is bounded by the corresponding indicator and therefore
$\E[\hat{p}_h^{\,b}(x)] \leq \Prob(g(x,\xi) \leq 0)$. The inequality is a
fixed-decision expectation statement. Because an optimized decision depends on
the same samples, it is not a finite-sample feasibility certificate without a
selection-aware concentration argument or independent validation.
...
optimism Chapter~4 measured), while the biased surrogate sits entirely
...
formula of Chapter~4, the closed-form constraint, the analytic gradient,
the constraint count independent of $N$ (while evaluation cost is $O(N)$), survives with $-g(x,\xi_j)$ replaced
by $-g(x,\xi_j) - b(h)$. In the language of the two error sources
separated in Chapter~4, the bias acts on the smoothing error alone: the
...
\begin{figure}[t]
  \centering
  \includegraphics[width=\textwidth]{kap05/figures/fig_domination.pdf}
  ...
  (shaded right), the optimism measured in Chapter~4; the biased
...
\subsubsection{Exact one-sided kernels}
\label{sec:exactbiasedkernels}

Condition \cref{eq:domination} is a constraint on kernel tails, and the
three kernels of Chapter~3 meet it in three instructively different
...
constraints by \citet{keil2021}, satisfies the domination condition exactly for appropriate parameter
choices: its compact, one-sided structure is designed around
\cref{eq:domination} rather than retrofitted to it, and its integrated form is
smooth and available in closed form. It is the literature reference whenever
an exact pointwise envelope is required; it is not implemented in the local
runner. The \emph{Epanechnikov kernel with bias} starts from the
AMISE-optimal kernel of Chapter~3 and exploits its compact support: with
...
of Chapter~3 carrying over into the optimization.

\subsubsection{Approximate conservatism with Gaussian tails}
\label{sec:approxgaussian}
...
\subsubsection{Reformulation workflow}
\label{sec:biasedworkflow}
...
$y_j(x) = g(x,\xi_j)$ of Chapter~4. Third, evaluate the biased
...
Chapter~4: one smooth inequality whose row count is independent of $N$,
with evaluation and gradient cost proportional to $N$.

\subsubsection{Observed safety--cost trade-off}
\label{sec:safetycosttradeoff}

What the bias changes is the trade, and \cref{fig:safetycost} prices it
on a standalone lognormal scalar diagnostic where the truth is computable. The comparison follows the question posed at the end of Chapter~4: the unbiased reformulation is smooth and less conservative, while the
biased reformulation is more conservative and has the fixed-decision
pointwise envelope described above; neither statement alone is a
selection-aware finite-sample certificate for an optimized decision.
...
trials over the tested range, with no monotone improvement across sample sizes; the biased certificate fails in $14\%$ of trials at
$N = 50$, falls to $5\%$ by $N = 200$, and is below $1\%$ from
$N = 500$. These observed failures are compatible with sampling error,
but the experiment does not provide a selection-aware finite-sample
guarantee. The premium paid for this is $2.6$ to $4\%$ of
objective value, shrinking as $N$ grows. The results quantify a bounded objective premium in this experiment and a
lower observed failure rate for the biased surrogate. They do not establish
that the ranking holds for other distributions or data-dependent decisions.

\begin{figure}[t]
  \centering
  \includegraphics[width=\textwidth]{kap05/figures/fig_safetycost.pdf}
  \caption{The price and the purchase of conservatism, on a standalone
  lognormal scalar diagnostic ($\varepsilon = 0.05$, $800$ trials per sample size, bands are
  ...
  the tested unbiased rates show no monotone improvement across sample sizes; the
  biased rate decreases in this diagnostic, falling below $1\%$ from
  $N = 500$; this empirical trend is not a finite-sample guarantee. The
  average objective premium paid for safety is
...
\subsubsection{Boole risk allocation}
\label{sec:booleallocation}
...
\subsubsection{Multivariate residual KDE}
...
curse of dimensionality of Chapter~3 now acts in the constraint
dimension $m$,
and the sample sizes it demands grow exponentially.

\subsubsection{Exact scalar violation statistic}
\label{sec:scalarviolation}
...
well as in $d$. The single catch is the one Chapter~3 anticipated: the
max in \cref{eq:scalarstat} is nonsmooth, and a nonsmooth residual would
re-break exactly the differentiability that Chapter~4 worked to
...
\subsubsection{Safe-side max approximation}
\label{sec:lsesafebound}
...
\subsubsection{Softmax gradient and joint reformulation}
\label{sec:lsegradient}
...
\end{equation}
and \cref{eq:jointreform} is an implemented extension and diagnostic, a claim supported by what the formulation absorbs: a
joint,
...
with two stacked approximation layers: the biased kernel supplies a fixed-decision expectation envelope, while log-sum-exp supplies a deterministic upper bound on the maximum. The
interaction of these layers, including how conservatism compounds across $h$ and $\tau$, remains unresolved; Chapter~9 reports a joint-event diagnostic rather than a risk-allocation study.

\begin{figure}[t]
  \centering
  \includegraphics[width=\textwidth]{kap05/figures/fig_lse.pdf}
...
\subsubsection{Continuation schedule}
\label{sec:continuationschedule}
...
hardest: a sharply smoothed constraint approaches the piecewise-constant empirical estimator of
...
keeps pace with geometric resolution; and near convergence, switch to the locally implemented shifted
Epanechnikov surrogate. The split-Bernstein switch shown in the cited workflow
is a literature procedure and is not executed by the local runner.

\begin{figure}[t]
  \centering
  \includegraphics[width=0.82\textwidth]{kap05/figures/fig_continuation.pdf}
  ...
  and the sample size grows; the cited final stage switches to the split-Bernstein kernel; that
  literature step is not executed by the local runner. The homotopy stabilizes the solver and
  avoids the numerical difficulty of attacking a small-bandwidth,
  nearly nonsmooth constraint from a cold start.}
  \label{fig:continuation}
\end{figure}

\subsubsection{Scope of the solver claim}
\label{sec:continuationscope}
...
method: a smooth, one-dimensional reformulation with constraint count
independent of $N$ for individual and joint chance constraints, equipped with
...
the true ones. The convergence theory of \citet{schuster2025}, and connecting it to the
